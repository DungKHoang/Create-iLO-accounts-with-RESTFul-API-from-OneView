...
if ([string]::IsNullOrWhiteSpace($iLOPassword))
{
    write-host -foreground YELLOW " Please provide iLO username/ password for new account to be created...."
    exit
}
...
$accData            = Get-HPRESTDataRaw -Href 'rest/v1/AccountService/Accounts' -Session $ilosession
$accUri             = $accData.'@odata.id'

# Get iLO user accounts
$ListofMembersIds   = $accData.Members | % { Get-HPRESTDataRaw -href $_.'@odata.id' -Session $iloSession }
$ListofUsers        = $ListofMembersIds | % { $_.Username}

if ($ListofUsers -notcontains $iLOuserName)
{
$ret                = Invoke-HPRESTAction -Href $accURI -Data $user -Session $ilosession
}
else 
{
write-host -foreground YELLOW "username --> $iLOuserName already exists"
